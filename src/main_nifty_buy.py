...
from strategy import strike_nif_buy
import pymongo
from confi import item2,market_key2,market_secret2,order_key2,order_secret2
...
def main(item,market_key,market_secret,order_key,order_secret):
    ...
    if len(traded)==0:
        if abs(df["open"][-1:].values[0] - df["close"][-2:-1].values[0]) >0.0039*df["close"][-2:-1].values[0] and datetime.now(tz= tzinfo).time() < time(9, 30, 0):
            ...
            tradable_buy.extend( strike_nif_buy(item,nearest_expiry,monthend_expiry,exchange, xts, day_delta))
        else:
            tradable_buy.extend( strike_nif_buy(item,nearest_expiry,monthend_expiry,exchange, xts, day_delta))
        print(now,"\n","BUY",tradable_buy)
        try:
            ...
            if datetime.now(tz= tzinfo).time() >= time(9, 15, 0):
                ...
                if traded:
                    for position in traded:
                        ...
                        if datetime.now(tz= tzinfo).second in range(5):
