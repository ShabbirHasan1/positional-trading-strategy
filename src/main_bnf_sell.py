...
from strategy import strike_bnf_sell
import pymongo
from config import item,market_key,market_secret,order_key,order_secret
...
def main(item,market_key,market_secret,order_key,order_secret):
    ...
    if len(traded)==0:    
        if abs(df["open"][-1:].values[0] - df["close"][-2:-1].values[0]) >0.0039*df["close"][-2:-1].values[0] and datetime.now(tz= tzinfo).time() < time(9, 30, 0):
            ...
            tradable_sell.extend(strike_bnf_sell(item,nearest_expiry,monthend_expiry,exchange, xts, day_delta))
        else:
            tradable_sell.extend(strike_bnf_sell(item,nearest_expiry,monthend_expiry,exchange, xts, day_delta))
        print(now,"\n","SELL",tradable_sell)
        try:
